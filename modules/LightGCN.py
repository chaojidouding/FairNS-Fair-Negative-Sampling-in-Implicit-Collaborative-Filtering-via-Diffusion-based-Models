...
class LightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        self.d_weight = args_config.d_weight
        ...
        self.run_type = args_config.run_type
    ...
    def get_embedding(self,batch):
        ...
        user_gcn_emb = user_gcn_emb.unsqueeze(dim=1)
        item_gcn_emb = item_gcn_emb.unsqueeze(dim=1)
        return user_gcn_emb, item_gcn_emb
    def forward(self, cur_epoch, batch=None):
        user = batch['users']
        user_emb = batch['users_emb']
        pos_item_emb = batch['pos_items_emb']
        neg_item_emb = batch['neg_items_emb']
        return self.create_bpr_loss(user, user_emb, pos_item_emb, neg_item_emb)
    ...
    def create_bpr_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        ...
        neg_e = self.pooling(neg_gcn_embs.reshape(-1, neg_gcn_embs.shape[2], neg_gcn_embs.shape[3])).view(batch_size, self.K, -1)
        ...
        if self.run_type == 1:
            tot = 1 + 0.9 + 0.8 + 0.7
            w = [self.d_weight*1/tot,self.d_weight*0.9/tot,self.d_weight*0.8/tot,self.d_weight*0.7/tot,1]
        elif self.run_type == 2:
            w = [1]
        elif self.run_type == 3:
            w = [1/tot,0.9/tot,0.8/tot,0.7/tot]
        else:
            w = [self.d_weight,1]
        w_tensor = torch.tensor(w, device=neg_scores.device).unsqueeze(0)  # shape: [1, K]
        weighted_diff = torch.sigmoid(pos_scores.unsqueeze(dim=1)-neg_scores)*w_tensor
        # calc loss
        mf_loss = torch.mean(-torch.log(weighted_diff).sum(dim=1))
        # cul regularizer
