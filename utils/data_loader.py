...
dataset_list = ['ml-2-types','tenrec-4','ml-4-types','tenrec-2']
...
def read_item(file_name):
    ...
    for l in lines:
        ...
        u_id, pos_ids = inters[0], inters[1:]
        for i_id in pos_ids:
            if i_id not in type_list:
                type_list.append(i_id)
        if u_id not in item_type:
            item_type[u_id] = pos_ids
    return item_type,len(type_list)
...
def read_item(file_name):
    ...
    for l in lines:
        ...
        if u_id not in item_type:
            item_type[u_id] = pos_ids
        for i_id in pos_ids:
            if i_id not in type_count:
                type_count[i_id] = 0
            type_count[i_id] += 1

    max_type_id = max(type_count.keys(), default=-1)
    type_list = [0] * (max_type_id + 1)
    for i_id, count in type_count.items():
        type_list[i_id] = count

    return item_type, (max_type_id + 1),type_list
...
def statistics(train_data, valid_data, test_data):
    ...
    n_pairs = len(train_data) + len(test_data)
...
def load_data(model_args):
    ...
    logger.info('reading train and test user-item set ...')
    train_cf = read_cf(directory + 'train.txt')
    test_cf = read_cf(directory + 'test.txt')

    item_type,total_types,type_num_list = read_item(directory + 'item.txt')
    ...
    return train_cf, user_dict, n_params, norm_mat,item_type,total_types,type_num_list
